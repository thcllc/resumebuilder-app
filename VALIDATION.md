...
When the release audit fails, its JSON output includes `blockers` and `nextActions`. Use those fields as the authoritative machine-readable reason the project is not complete, including missing or unreadable receipt input paths.
